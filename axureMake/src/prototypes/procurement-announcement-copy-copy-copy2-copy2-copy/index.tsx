...
import aiParsedTag from '../../../assets/picture/image copy.png';
import {
  Home,
  UserPlus,
  ShoppingCart,
...
const Component = () => {
  ...
  // 资审条件智能推荐表格数据
  const [qualConditionRecommendData, setQualConditionRecommendData] = useState([
  ...
  // 附件组成智能推荐表格数据
  const [attachmentRecommendData, setAttachmentRecommendData] = useState([
  ...
  // 在资格审查条件树中按标题查找节点
  const findTreeNodeByTitle = (nodes: TreeDataNode[], title: string): { title: string; key: string } | null => {
    for (const node of nodes) {
      if (node.title === title) {
        return { title: node.title as string, key: node.key as string };
      }
      if (node.children) {
        const found = findTreeNodeByTitle(node.children, title);
  ...
  const handleFileAttachChange = (rowId: number, value: string) => {
    setQualConditions((prev) =>
      prev.map((item) =>
        item.id === rowId ? { ...item, category: value, aiParsed: false } : item
      )
    );
  };
  ...
  // 一键引用推荐附件组成到下方表格
  const handleOneClickQuoteAttachments = () => {
    const toQuote = attachmentRecommendData.filter((item) => item.action === '引用');
    if (toQuote.length === 0) return;

    const newAttachments = toQuote.map((item, index) => ({
      id: Date.now() + index,
      name: item.fileItem === '自定义' ? '自定义' : item.fileItem,
      fileName: item.fileItemName,
    ...
    setAttachmentRecommendData((prev) =>
      prev.map((item) => (item.action === '引用' ? { ...item, action: '已设置' } : item))
    );
  };

  // 一键引用推荐资审条件到下方表格
  const handleOneClickQuoteQualConditions = () => {
    const toQuote = qualConditionRecommendData.filter((item) => item.action === '引用');
    if (toQuote.length === 0) return;

    const newConditions = toQuote.map((item, index) => {
      const displayName = item.conditionName.split('\n')[0];
      const matched =
        findTreeNodeByTitle(defaultConditionTreeData, item.conditionItem) ||
        findTreeNodeByTitle(defaultConditionTreeData, displayName);

      if (matched) {
        const isFileDetect = matched.key.startsWith('file-detect');
        return {
          id: Date.now() + index,
          name: matched.title,
          treeKey: matched.key,
    ...
    setQualConditions((prev) => [...prev, ...newConditions]);
    setQualConditionRecommendData((prev) =>
      prev.map((item) => (item.action === '引用' ? { ...item, action: '已设置' } : item))
    );
  };
                ...
                <div className="flex items-center">
                  ...
                  {showAttachmentRecommendation && (
                    <div className="ml-3 relative">
                      ...
                      >智能附件组成推荐，点击查看！</span>
                      <span className="text-xs text-gray-900 dark:text-gray-100 ml-1">或</span>
                      <span
                        className={`text-xs ml-1 ${
                          attachmentRecommendData.some((item) => item.action === '引用')
                            ? 'text-primary cursor-pointer hover:underline'
                            : 'text-gray-400 cursor-not-allowed'
                        }`}
                        onClick={() => {
                          if (attachmentRecommendData.some((item) => item.action === '引用')) {
                            handleOneClickQuoteAttachments();
                          }
                        }}
                      >一键引用</span>
                    </div>
                        ...
                        },
                        {
                          ...
                          render: (text: string, record: any) => (
                            <div className="relative">
                              ...
                              />
                              {record.aiParsed && (
                                <img src={aiParsedTag} alt="AI解析" className="h-4 inline-block absolute -top-1 -right-0.5" />
                              )}
                            </div>
                          ),
                        },
            ...
            <div className="bg-white dark:bg-gray-800 rounded shadow-sm mb-10 border border-gray-200 dark:border-gray-700">
              <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <span
                    className={`text-xs ml-1 ${
                      qualConditionRecommendData.some((item) => item.action === '引用')
                        ? 'text-primary cursor-pointer hover:underline'
                        : 'text-gray-400 cursor-not-allowed'
                    }`}
                    onClick={() => {
                      if (qualConditionRecommendData.some((item) => item.action === '引用')) {
                        handleOneClickQuoteQualConditions();
                      }
                    }}
                  >一键引用</span>
                </div>
                      ...
                      columns={[
                        ...
                        },
                        {
                          ...
                          render: (text: string, record: any) => {
                            ...
                            return <span className="text-xs text-gray-700 dark:text-gray-200 whitespace-pre-line">{text}</span>;
                          },
                        },
                        {
                          title: '资格审查条件分类',
            ...
            <div className="flex-1 overflow-auto p-4">
              <div className="flex justify-end mb-2">
                <button
                  ...
                  onClick={handleOneClickQuoteAttachments}
                  disabled={!attachmentRecommendData.some((item) => item.action === '引用')}
                >
                  一键引用
                </button>
              </div>
                ...
                <>
                  <div className="flex justify-end mb-2">
                    <button
                      ...
                      onClick={handleOneClickQuoteQualConditions}
                      disabled={!qualConditionRecommendData.some((item) => item.action === '引用')}
                    >
                      一键引用
                    </button>
                  </div>
