...
const Component = () => {
  ...
  const [checkModalAttachmentId, setCheckModalAttachmentId] = useState<number | null>(null);

  const toggleAttachmentCheck = (attachmentId: number, checkItem: string) => {
    setAttachmentCheckSettings((prev) => {
      const current = prev[attachmentId] || [];
      if (current.includes(checkItem)) {
        return { ...prev, [attachmentId]: current.filter((c) => c !== checkItem) };
      }
      return { ...prev, [attachmentId]: [...current, checkItem] };
    });
  };
  ...
  return (
    <div className="bg-background-light dark:bg-background-dark text-gray-800 dark:text-gray-200 font-sans h-screen flex overflow-hidden">
                ...
                {isQualAttachmentSet && (
                  <div className="flex items-center mb-4">
                    ...
                    <Radio.Group
                      ...
                      onChange={(e: any) => setIsAIFileDetectSet(e.target.value)}
                    >
                      <Radio value={true}>是</Radio>
                      <Radio value={false}>否</Radio>
                    </Radio.Group>
                        ...
                        ...(isAIFileDetectSet ? [
                          {
                            ...
                            dataIndex: 'id',
                            render: (id: number) => {
                              const selected = attachmentCheckSettings[id] || [];
                              return (
                                <div className="relative">
                                  <button
                                    className="flex items-center text-primary border border-primary rounded px-2 py-0.5 text-xs bg-white dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-blue-900/20"
                                    onClick={() => setCheckModalAttachmentId(id)}
                                  >
      ...
      {/* 检测项设置弹窗 */}
      {checkModalAttachmentId && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-[60]">
          <div className="bg-white dark:bg-gray-800 w-full max-w-md rounded-lg shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            {/* 弹窗头部 */}
            <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between bg-gray-50 dark:bg-gray-800">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-bold text-gray-800 dark:text-gray-100">
                  检测项设置 - {attachments.find(a => a.id === checkModalAttachmentId)?.name}
                </h2>
              </div>
              <button
                className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors"
                onClick={() => setCheckModalAttachmentId(null)}
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-auto custom-scrollbar p-6">
              {(() => {
                const selected = attachmentCheckSettings[checkModalAttachmentId] || [];
                return (
                      ...
                      <div key={category.name}>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {category.items.map((item) => (
                            <label
                              key={item}
                              className="flex items-center p-3 border border-gray-200 dark:border-gray-700 rounded hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition-colors"
                            >
                              <input
                                type="checkbox"
                                className="rounded border-gray-300 text-primary focus:ring-primary mr-3 w-4 h-4"
                                checked={selected.includes(item)}
                                onChange={() => toggleAttachmentCheck(checkModalAttachmentId, item)}
                              />
            ...
            <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center">
              <span className="text-xs text-gray-500">
                已选 {(attachmentCheckSettings[checkModalAttachmentId] || []).length} 项
              </span>
              <button
