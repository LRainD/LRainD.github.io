...
const Component = function EnterpriseRelationConfig() {
  ...
  const renderActionCell = (_: any, record: RelationViewRow) => {
    const content = isViewMode ? (
      <span className="template-action-delete-disabled">删除</span>
    ) : (
      <span className="template-action-delete" onClick={() => handleDeleteRelation(record.relationId)}>删除</span>
    );
    return renderMergedCell(content, record.isFirstScene);
  };
    ...
    >
      <div className="operation-template-mgmt-page">
        <div className="enterprise-relation-toolbar">
          <Button type="link" icon={<PlusOutlined />} onClick={handleAddRelation} disabled={isViewMode}>新增企业关系</Button>
          <Button type="primary" onClick={handleSubmit} disabled={isViewMode}>提交</Button>
        </div>
        <Table
