...
const hasProcessingTimeLimit = (status: string) => status.endsWith('待处理') || status.endsWith('中') || status.includes('回退审批中');
...
const createTransitionRecords = (type: TaskType, status: string, sequence: number): TransitionRecord[] => {
  ...
  const addCompletedStage = (stage: TaskType, withIssues: boolean) => {
    ...
    if (!withIssues) {
      add('检查提交', `${stage}检查结论为合规`, processingStatus, completeStatus, inspector, '全部检查项判定合规。');
      return;
    }
    add('检查提交', `${stage}检查发现不合规项`, processingStatus, dispositionStatus, inspector, '发现不合规检查项，已生成处置待办。');
...
const getTaskAction = (status: string) => {
  if (status.includes('申诉')) return '查看申诉';
...
type AppealRound = { content: string; attachments: string[] };
...
const MyTasks: React.FC = () => {
  ...
  const handleTaskAction = (record: typeof mockTasks[number]) => {
    ...
    setSelectedMode(record.status.includes('申诉') ? '申诉查看' : getTaskAction(record.status));
    setResults({});
    setItemRemarks({});
    setItemAttachments({});
  ...
  const isAppealView = selectedMode === '申诉查看';
  const checkItems = selectedTaskId ? taskCheckItems[selectedTaskId] || defaultCheckItems : defaultCheckItems;
  ...
  const actionTitle = useMemo(() => selectedTask ? `${selectedTask.type}${isAppealView ? '申诉详情' : selectedMode}` : '', [isAppealView, selectedMode, selectedTask]);
  ...
  const handleSubmit = () => {
    ...
    if (isAppealing) {
      ...
      if (appealItemKeys.some(key => !appealDrafts[key]?.trim())) {
        message.warning('请填写全部已选检查项的申诉说明');
        return;
      }
      if (appealItemKeys.some(key => !appealAttachments[key])) {
        message.warning('请上传全部已选检查项的申诉附件');
        return;
      }
      if (!selectedTask) return;
      const submittedAt = dayjs().format('YYYY-MM-DD HH:mm:ss');
      const appealRounds = Object.fromEntries(appealItemKeys.map(key => [key, [{ content: appealDrafts[key], attachments: [appealAttachments[key]] }]]));
  ...
  const executionColumns = [
    ...(isAppealing || isAppealView ? [{
      ...
      render: (_: unknown, record: CheckItem) => {
        ...
        if (isAppealView) return existingAppeal ? <Checkbox checked disabled /> : <span>-</span>;
    ...
    { title: '检查标准', dataIndex: 'standard', key: 'standard', width: 310 },
    {
      ...
      render: (_: unknown, record: CheckItem) => {
        ...
        const result = isInspection ? systemResults[record.key] : record.result;
    ...
    ...(isAppealing || isAppealView ? [
      {
        ...
        render: (_: unknown, record: CheckItem) => {
          ...
          if (isAppealView) return <span>{appeal?.content || '-'}</span>;
          if (!appealItemKeys.includes(record.key)) return <span>-</span>;
      ...
      },
      {
        ...
        render: (_: unknown, record: CheckItem) => {
          const appeal = appealsByTask[selectedTaskId || '']?.[record.key]?.[0];
          if (isAppealView) return appeal?.attachments.length ? <Space direction="vertical" size={0}>{appeal.attachments.map(file => <Button key={file} type="link" icon={<PaperClipOutlined />}>{file}</Button>)}</Space> : <span>-</span>;
          if (!appealItemKeys.includes(record.key)) return <span>-</span>;
    ...
    ...(!isInspection && !isAppealing && !isAppealView ? Array.from({ length: roundCount + (isRectification ? 1 : 0) }, (_, index) => {
        ...
        <div className="my-tasks-page">
          {selectedTask ? <>
            ...
            <TopActionBar
              ...
              actions={(isInspection || isRectification || isReview || isAppealing) && <Space>{isRectification && <Button type="default" onClick={() => setSelectedMode('申诉')}>申诉</Button>}<Button icon={<SaveOutlined />} onClick={() => message.success('草稿已保存')}>保存草稿</Button><Button type="primary" icon={<CheckCircleOutlined />} onClick={handleSubmit}>{isInspection ? '提交检查' : isRectification ? '提交整改' : isAppealing ? '提交申诉' : '提交复核'}</Button></Space>}
            />
            ...
            <Card className="task-execution-card" title={isInspection ? '检查执行' : isAppealing || isAppealView ? '申诉相关信息' : '检查项处置明细'}>
              ...
              {isAppealing && <Alert className="task-execution-alert" type="warning" showIcon message="请选择需要申诉的不合规检查项，并填写第1轮申诉说明、上传申诉附件。" />}
              {isAppealView && <Alert className="task-execution-alert" type="info" showIcon message="以下为当前任务已提交的申诉信息，申诉材料将由局管理员处理。" />}
