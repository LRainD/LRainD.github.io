...
const Component = () => {
  ...
  const [isAIRecommendModalOpen, setIsAIRecommendModalOpen] = useState(false);
  ...
  // AI智能推荐字段配置（包含推荐值）
  const aiFields = [
    { id: 'autoPublish', label: '是否自动发布公告', recommendValue: '是' },
    { id: 'hoursAfterApproval', label: '预计报名截止时间', recommendValue: '48小时' },
    { id: 'noticeFormat', label: '采购公告格式', recommendValue: '范本' },
    { id: 'noticeContent', label: '公告内容', recommendValue: '使用AI生成内容' },
  ];
  ...
  const handleAIRecommend = () => {
    ...
    setTimeout(() => {
      ...
      setIsAIRecommendModalOpen(true);
      // 默认全选
      setSelectedAIFields(aiFields.map(f => f.id));
  ...
  const handleFieldSelect = (fieldId: string) => {
    if (selectedAIFields.includes(fieldId)) {
      setSelectedAIFields(prev => prev.filter(id => id !== fieldId));
      setSelectAllFields(false);
    } else {
      setSelectedAIFields(prev => [...prev, fieldId]);
      if (selectedAIFields.length + 1 === aiFields.length) {
        setSelectAllFields(true);
      }
    }
  };
  ...
  const handleConfirmAIRecommend = () => {
    // 应用选中的推荐值到表单
    selectedAIFields.forEach(fieldId => {
      const field = aiFields.find(f => f.id === fieldId);
      if (field) {
        switch (fieldId) {
          case 'autoPublish':
            setAutoPublish(field.recommendValue === '是');
            break;
          case 'hoursAfterApproval':
            setHoursAfterApproval('48');
            break;
          // noticeFormat 和 noticeContent 是UI展示字段，实际项目中可能需要更复杂的处理
        }
      }
    });
    setIsAIRecommendModalOpen(false);
    setSelectedAIFields([]);
    setSelectAllFields(false);
  };
    ...
    <div className="bg-background-light dark:bg-background-dark text-gray-800 dark:text-gray-200 font-sans h-screen flex overflow-hidden">
      ...
      {isAIRecommending && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center">
          <div className="bg-white dark:bg-gray-800 rounded-xl px-8 py-6 shadow-2xl flex flex-col items-center">
            <Loader2 className="w-10 h-10 text-purple-500 animate-spin mb-4" />
            <p className="text-lg font-medium text-gray-800 dark:text-gray-200">智能推荐中</p>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">请稍候，正在为您生成推荐内容...</p>
          </div>
        </div>
      )}

      {/* AI智能推荐弹窗 */}
      {isAIRecommendModalOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 w-full max-w-2xl rounded shadow-lg overflow-hidden flex flex-col max-h-[80vh]">
            {/* 弹窗头部 */}
            <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-1 h-5 bg-blue-500"></div>
                <h2 className="text-base font-medium text-gray-800 dark:text-gray-100">智能推荐</h2>
              </div>
              <button
                className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors"
                onClick={() => setIsAIRecommendModalOpen(false)}
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-auto p-4">
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                系统已为您生成以下推荐字段，请选择需要应用的推荐值：
              </p>
              <table className="w-full text-left border-collapse">
                <thead className="sticky top-0 z-10">
                  <tr className="bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm">
                    <th className="px-3 py-2 font-normal border-b border-gray-200 dark:border-gray-600 w-12 text-center">
                      <input
                        type="checkbox"
                    ...
                    <th className="px-3 py-2 font-normal border-b border-gray-200 dark:border-gray-600">
                      字段名称
                    </th>
                    <th className="px-3 py-2 font-normal border-b border-gray-200 dark:border-gray-600">
                      推荐值
                      ...
                      <td className="px-3 py-2 text-gray-600 dark:text-gray-400">
                        {field.id === 'noticeContent' ? (
                          <div className="inline-flex items-center border border-gray-200 dark:border-gray-600 rounded px-3 py-1.5 bg-white dark:bg-gray-700">
                            ...
                            <span className="text-gray-700 dark:text-gray-300">lrd测试模版.pdf</span>
                          </div>
                        ) : (
            ...
            <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-2">
              <button
                className="bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 dark:bg-gray-700 dark:border-gray-600 dark:hover:bg-gray-600 dark:text-gray-200 px-5 py-1.5 rounded text-sm transition-all"
                onClick={() => setIsAIRecommendModalOpen(false)}
              >
                取消
              </button>
              <button
                className="bg-blue-500 hover:bg-blue-600 text-white px-5 py-1.5 rounded text-sm transition-all"
