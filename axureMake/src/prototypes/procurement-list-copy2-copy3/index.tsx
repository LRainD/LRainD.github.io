...
const Component = () => {
  ...
  // AI推荐时间线状态: 'pending' | 'loading' | 'completed'
  const [aiFieldStatuses, setAiFieldStatuses] = useState<{ [key: string]: 'pending' | 'loading' | 'completed' }>({});
  ...
  const handleAIRecommend = () => {
    ...
    setShowAITimeline(true);
    
    // 初始化所有字段状态为 pending，第一个为 loading
    const initialStatuses: { [key: string]: 'pending' | 'loading' | 'completed' } = {};
    aiFields.forEach((field, index) => {
      initialStatuses[field.id] = index === 0 ? 'loading' : 'pending';
    });
    setAiFieldStatuses(initialStatuses);
    setAiRecommendationProgress(0);

    // 使用 ref 来存储当前索引，避免闭包问题
    let indexRef = 0;
    
    // 模拟逐个字段填充
    const interval = setInterval(() => {
      if (indexRef >= aiFields.length) {
        clearInterval(interval);
        setIsAIRecommending(false);
        return;
      }

      const currentField = aiFields[indexRef];
      const nextIndex = indexRef + 1;

      // 更新字段状态
      setAiFieldStatuses((prev: { [key: string]: 'pending' | 'loading' | 'completed' }) => {
        const newStatuses = { ...prev };
        // 当前字段变为 completed
        newStatuses[currentField.id] = 'completed';
        // 下一个字段变为 loading（如果存在）
        if (nextIndex < aiFields.length) {
          newStatuses[aiFields[nextIndex].id] = 'loading';
        }
        return newStatuses;
      });

      setAiRecommendationProgress(((indexRef + 1) / aiFields.length) * 100);
      indexRef++;
    }, 1500); // 每个字段停留1.5秒
  };
  
  // 关闭AI时间线面板
  const handleCloseAITimeline = () => {
  ...
  return (
    <div className="bg-background-light dark:bg-background-dark text-gray-800 dark:text-gray-200 font-sans h-screen flex overflow-hidden">
      ...
      {/* AI智能推荐时间线面板 */}
      {showAITimeline && (
        <div className="fixed right-4 top-20 w-[180px] bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-purple-100 dark:border-purple-800 z-50 overflow-hidden">
          {/* 头部 */}
          <div className="bg-gradient-to-r from-purple-500 to-blue-500 px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-white" />
              <span className="text-sm font-semibold text-white">智能推荐</span>
            </div>
            {!isAIRecommending && (
              <button
                onClick={handleCloseAITimeline}
          ...
          <div className="px-4 py-2 bg-purple-50 dark:bg-purple-900/20">
            <div className="flex items-center justify-between text-xs text-gray-600 dark:text-gray-400 mb-1">
              <span>推荐进度</span>
              <span>{Math.round(aiRecommendationProgress)}%</span>
            </div>
          ...
          <div className="p-4 max-h-[300px] overflow-y-auto">
            <div className="relative">
              ...
              {aiFields.map((field, index) => {
                const status = aiFieldStatuses[field.id] || 'pending';
                return (
                  <div key={field.id} className="relative flex items-start gap-3 mb-4 last:mb-0">
                    {/* 状态图标 */}
          ...
          <div className="px-4 py-2 bg-gray-50 dark:bg-gray-700/50 border-t border-gray-100 dark:border-gray-700">
            <p className="text-xs text-gray-500 dark:text-gray-400 text-center">
              {isAIRecommending 
                ? '正在智能推荐中...' 
                : `推荐完成，已为您填充${aiFields.length}个字段，已跳过${filledFieldsCount}个字段`}
            </p>
          </div>
        </div>
      )}
