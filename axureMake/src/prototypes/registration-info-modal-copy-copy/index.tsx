...
const Component = () => {
  ...
  // 独立存储两种方式的附件
  const [esignAttachments, setEsignAttachments] = useState<Attachment[]>([
    { id: 1, name: '项目采购文件评审.pdf' }
  ]);
  const [scanAttachments, setScanAttachments] = useState<Attachment[]>([]);

  // 当前显示的附件列表
  const currentAttachments = uploadMethod === 'esign' ? esignAttachments : scanAttachments;
  const setCurrentAttachments = uploadMethod === 'esign' ? setEsignAttachments : setScanAttachments;
  ...
  const handleRemoveAttachment = (id: number) => {
    setCurrentAttachments(prev => prev.filter(item => item.id !== id));
  };
  ...
  const handleAddAttachment = () => {
    if (uploadMethod === 'esign') {
      ...
      setCurrentAttachments(prev => [...prev, { id: newId, name: `本地上传附件_${newId}.pdf` }]);
    }
  };
  ...
  // 检查是否有签署中的附件
  const hasSigningAttachments = currentAttachments.some(att => att.signingStatus === 'signing');
                ...
                <div className="pl-1 space-y-4">
                  ...
                  {uploadMethod && (
                    <>
                      ...
                      {/* 附件列表 */}
                      <div className="ml-[100px] flex flex-wrap gap-3">
                        {currentAttachments.map((file) => (
                          ...
                          >
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-[10px] text-gray-400">2.4MB</span>
                            </div>
          ...
          <div className="flex items-center justify-end px-5 py-4 border-t border-gray-100 gap-3">
            ...
            {uploadMethod === 'esign' && (
              <button
                ...
                onClick={() => {
                  // 模拟发起电子签逻辑
                  setEsignAttachments(prev => prev.map(a => ({ ...a, signingStatus: 'signed' })));
                  handleSubmit();
                }}
              >
                发起电子签后报名
