...
import { Table, Switch, Radio, Select } from 'antd';
import './style.css';
...
const Component = () => {
  ...
  const toggleRequired = (id: number) => {
    setAttachments((prev: typeof attachments) => {
      const updated = prev.map((item: typeof attachments[0]) =>
        item.id === id ? { ...item, required: !item.required } : item
      );
      // 联动：将 required 为 true 的文件项同步到 AI 辅助报名文件检测表格
      const requiredNames = updated
        .filter((item) => item.required)
        .map((item) => item.name);
      setAIAssistFileDetections((prevDetections) => {
        // 保留用户手动添加的自定义项
        const customItems = prevDetections.filter((d) => d.category === '自定义');
        // 根据 required 的文件项生成新的来自公告的项
        const autoItems = requiredNames.map((name, index) => ({
          id: -1000 - index, // 使用负数 id 区分自动生成的项
          detectItem: name,
          category: '来自公告' as const,
        }));
  ...
  // AI辅助报名文件检测
  const [isAIAssistFileDetectSet, setIsAIAssistFileDetectSet] = useState(true);
  const [aiAssistFileDetections, setAIAssistFileDetections] = useState([
    { id: 1, detectItem: '营业执照附件', category: '来自公告' },
    { id: 2, detectItem: '法人身份证附件', category: '来自公告' },
    { id: 3, detectItem: '资质证书材料附件', category: '来自公告' },
    { id: 4, detectItem: '业绩证明资料附件', category: '来自公告' }
  ]);

  const DETECT_ITEM_OPTIONS = [
  ...
  const handleAddAIAssistFileDetection = () => {
    setAIAssistFileDetections((prev: typeof aiAssistFileDetections) => [
      ...prev,
      { id: Date.now(), detectItem: '', category: '自定义' }
    ]);
  };

  const handleDeleteAIAssistFileDetection = (id: number) => {
    setAIAssistFileDetections((prev: typeof aiAssistFileDetections) => prev.filter((item: typeof prev[0]) => item.id !== id));
  };

  const handleUpdateAIAssistFileDetection = (id: number, field: string, value: string) => {
    setAIAssistFileDetections((prev: typeof aiAssistFileDetections) => prev.map((item: typeof prev[0]) =>
            ...
            <div className="bg-white dark:bg-gray-800 rounded shadow-sm mb-10 border border-gray-200 dark:border-gray-700">
              ...
              <div className="p-4">
                ...
                <div className="flex items-center mb-4 mt-6">
                  <span className="text-gray-700 dark:text-gray-300 text-sm mr-4">是否使用AI辅助报名文件检测：</span>
                  <Radio.Group
                    value={isAIAssistFileDetectSet}
                    onChange={(e: any) => setIsAIAssistFileDetectSet(e.target.value)}
                  >
                    <Radio value={true}>是</Radio>
                    <Radio value={false}>否</Radio>
                  </Radio.Group>
                ...
                {/* AI辅助报名文件检测表格 */}
                {isAIAssistFileDetectSet && (
                  <>
                    <Table
                      dataSource={aiAssistFileDetections}
                      rowKey="id"
                      size="small"
                      pagination={false}
                      columns={[
                        ...
                        {
                          title: <><span className="text-red-500">*</span> 检测项名称</>,
                          ...
                          render: (text: string, record: any) => (
                            <Select
                              ...
                              style={{ width: '100%' }}
                              size="small"
                              onChange={(value: string) => handleUpdateAIAssistFileDetection(record.id, 'detectItem', value)}
                              options={DETECT_ITEM_OPTIONS.map((item) => ({ label: item, value: item }))}
                            />
                          ),
                        },
                        {
                          title: <><span className="text-red-500">*</span> 分类</>,
                          dataIndex: 'category',
                          render: (text: string) => (
                        ...
                        },
                        {
                          ...
                          render: (_: any, record: any) => (
                            <span
                              ...
                              onClick={() => handleDeleteAIAssistFileDetection(record.id)}
                            >
                              删除
                            </span>
                          ),
                    ...
                    <div
                      ...
                      onClick={handleAddAIAssistFileDetection}
                    >
                      <span className="text-gray-500 text-sm">添加</span>
                    </div>
                  </>
