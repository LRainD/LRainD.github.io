    ...
    <div className="bg-background-light dark:bg-background-dark text-gray-800 dark:text-gray-200 font-sans h-screen flex overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-gray-50 dark:bg-gray-900">
          ...
          <div className="bg-gray-100 dark:bg-gray-800 rounded-sm mb-4 border border-gray-200 dark:border-gray-700 flex text-sm">
          ...
          <div className="bg-white dark:bg-gray-800 p-8 mb-4 rounded border border-gray-200 dark:border-gray-700 relative">
      ...
      {/* Chat Modal */}
      {isChatOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-custom p-4">
          <div className="relative bg-white dark:bg-slate-900 w-full max-w-5xl rounded-[32px] shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-[650px] max-h-[90vh] animate-in fade-in zoom-in duration-300">
            <button className="absolute top-6 right-6 w-12 h-12 flex items-center justify-center rounded-full bg-slate-100/80 hover:bg-slate-200 dark:bg-slate-800/80 dark:hover:bg-slate-700 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100 transition-all duration-200 z-50 shadow-sm border border-white/20" onClick={() => setIsChatOpen(false)}>
              <X className="w-8 h-8" />
            </button>
            <div className="flex-1 flex flex-col bg-gradient-to-br from-slate-50 to-white dark:from-slate-800 dark:to-slate-900">
              <div className="px-10 py-6 border-b border-slate-100 dark:border-slate-700 flex items-center gap-3">
                <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center">
                  <ShieldCheck className="w-6 h-6 text-primary" />
                </div>
                <div>
                  <h2 className="text-xl font-bold text-slate-800 dark:text-white">数字合规官</h2>
              ...
              <div className="flex-1 overflow-y-auto p-10 chat-container" ref={chatHistoryRef}>
                {chatMessages.map((msg) => (
                  <div key={msg.id} className={`flex gap-4 mb-8 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
                    {msg.sender === 'bot' && (
                      <div className="flex-shrink-0">
                        <img alt="Digital Officer Mascot" className="w-10 h-10 rounded-full ring-2 ring-primary/20" src="https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=digital%20compliance%20officer%20mascot%20avatar&image_size=square" />
                      </div>
                    )}
                    {msg.sender === 'user' ? (
                      <div className="bg-primary text-white px-4 py-3 rounded-2xl rounded-tr-none max-w-[80%] shadow-sm">
                        {msg.text}
                      </div>
                    ) : msg.type === 'text' ? (
                      <div className="space-y-2">
                        <div className="bg-white dark:bg-slate-800 px-5 py-4 rounded-2xl rounded-tl-none shadow-sm border border-slate-100 dark:border-slate-700 max-w-[90%]">
                      ...
                      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex-1">
                        <div className="bg-white dark:bg-slate-800 px-5 py-4 rounded-2xl rounded-tl-none shadow-sm border border-slate-100 dark:border-slate-700">
                          <p className="font-semibold text-slate-800 dark:text-white mb-4">为您检测到以下合规预警节点：</p>
                          <div className="space-y-4">
                            {/* Node 1 */}
                            <div className="relative group bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-100 dark:border-slate-700 shadow-sm flex items-center justify-between cursor-pointer" onClick={() => toggleNode('node1')}>
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                  <h3 className="text-slate-800 dark:text-white font-medium">采购发起</h3>
                              ...
                              {expandedNodes['node1'] ? <ChevronUp className="w-5 h-5 text-slate-400" /> : <ChevronDown className="w-5 h-5 text-slate-400" />}
                            </div>
                            {expandedNodes['node1'] && (
                              <div className="pl-10 space-y-3 relative animate-in slide-in-from-top-2">
                                <div className="absolute left-4 top-0 bottom-4 w-px bg-slate-200 dark:bg-slate-700"></div>
                                <div className="relative bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-100 dark:border-slate-700 flex items-center justify-between cursor-pointer" onClick={() => setIsInterceptModalOpen(true)}>
                                ...
                                <div className="relative bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-100 dark:border-slate-700 cursor-pointer" onClick={() => setIsInterceptModalOpen(true)}>
                                  <div className="absolute -left-6 top-1/2 w-6 h-px bg-slate-200 dark:bg-slate-700"></div>
              ...
              <div className="px-10 pb-10 pt-4 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
                <div className="relative flex items-center gap-4">
                  <div className="flex-1 relative">
                    <input
                      className="w-full pl-6 pr-14 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl focus:ring-2 focus:ring-primary/20 text-slate-800 dark:text-slate-100 placeholder-slate-400"
                    ...
                    />
                    <button className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center rounded-xl bg-primary text-white hover:bg-primary/90 transition-colors" onClick={handleSendMessage}>
                      <Send className="w-[14px] h-[14px]" />
                    </button>
                  </div>
                </div>
                <div className="mt-4 flex gap-2 overflow-x-auto no-scrollbar pb-1">
                  <button className="whitespace-nowrap px-4 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 text-xs text-slate-500 hover:border-primary hover:text-primary transition-colors" onClick={() => { setChatInputValue('查看合规风险'); setTimeout(handleSendMessage, 100); }}>查看合规风险</button>
                  <button className="whitespace-nowrap px-4 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 text-xs text-slate-500 hover:border-primary hover:text-primary transition-colors" onClick={() => { setChatInputValue('项目状态查询'); setTimeout(handleSendMessage, 100); }}>项目状态查询</button>
                </div>
              </div>
            </div>
            <div className="hidden lg:flex w-80 p-10 flex-col justify-start bg-slate-50/50 dark:bg-slate-800/50 border-l border-slate-100 dark:border-slate-700">
              <div className="space-y-8">
                <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider">项目基本信息</h3>
                <div className="space-y-6">
                  <div>
                    <label className="block text-slate-400 text-xs mb-1">采购名称：</label>
                    <p className="text-slate-800 dark:text-slate-100 font-medium text-sm">XXX项目钢材采购</p>
                  </div>
                  <div>
                    <label className="block text-slate-400 text-xs mb-1">采购编号：</label>
                    <p className="text-primary hover:underline cursor-pointer break-all font-mono text-xs">cscec202602030000022364</p>
                  </div>
                  <div>
                    <label className="block text-slate-400 text-xs mb-1">组织机构：</label>
                    <p className="text-slate-800 dark:text-slate-100 font-medium text-sm">中建三局西南分公司</p>
                  </div>
                  <div>
                    <label className="block text-slate-400 text-xs mb-1">采购方式：</label>
                    <p className="text-slate-800 dark:text-slate-100 font-medium text-sm">竞价采购</p>
                  </div>
                  <div>
                    <label className="block text-slate-400 text-xs mb-1">采购项目：</label>
                    <p className="text-slate-800 dark:text-slate-100 font-medium text-sm">XXX采购项目</p>
                  </div>
                  <div>
                    <label className="block text-slate-400 text-xs mb-1">采购品类：</label>
                    <p className="text-slate-800 dark:text-slate-100 font-medium text-sm">物资-钢材</p>
                  </div>
                  <div>
                    <label className="block text-slate-400 text-xs mb-1">经办人：</label>
                    <p className="text-slate-800 dark:text-slate-100 font-medium text-sm">张三</p>
                  </div>
                </div>
              </div>
            </div>
