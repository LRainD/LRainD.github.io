...
const Component = () => {
  ...
  // 智能推荐漫游式引导状态
  const [showAITourGuide, setShowAITourGuide] = useState(true);
  const [aiTourGuideDismissed, setAiTourGuideDismissed] = useState(false);
  const aiTourButtonRef = useRef<HTMLButtonElement>(null);
  const [aiTourPosition, setAiTourPosition] = useState({ x: 0, y: 0 });
  ...
  // 关闭智能推荐漫游式引导
  const handleCloseAITourGuide = () => {
    setShowAITourGuide(false);
  };

  // 不再显示智能推荐漫游式引导
  const handleDismissAITourGuide = () => {
    setShowAITourGuide(false);
    setAiTourGuideDismissed(true);
  };

  // 计算漫游式引导弹窗位置
  useEffect(() => {
    if (showAITourGuide && aiTourButtonRef.current) {
      const rect = aiTourButtonRef.current.getBoundingClientRect();
      const popoverWidth = 320; // 弹窗宽度
      // 弹窗居中显示在按钮下方
      setAiTourPosition({
        x: rect.left + rect.width / 2 - popoverWidth / 2, // 按钮中心对齐
        y: rect.bottom + 12, // 按钮下方12px
      });
    }
  }, [showAITourGuide]);
  ...
  return (
    <div className="bg-background-light dark:bg-background-dark text-gray-800 dark:text-gray-200 font-sans h-screen flex overflow-hidden">
            ...
            <div className="bg-white dark:bg-gray-800 rounded shadow-sm mb-4 border border-gray-200 dark:border-gray-700">
              <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2 relative">
                  <button
                    ref={(el) => {
                      aiRecommendButtonRef.current = el;
                      aiTourButtonRef.current = el;
                    }}
                    onClick={() => {
                      handleCloseAITourGuide();
      ...
      {/* 智能推荐漫游式引导 */}
      {showAITourGuide && !aiTourGuideDismissed && (
        <div className="fixed inset-0 z-50">
          {/* 半透明背景遮罩 */}
          <div className="absolute inset-0 bg-black/40 transition-opacity duration-300"></div>
          {/* 引导弹窗 */}
          <div
            className="absolute bg-white rounded-lg shadow-2xl border border-gray-200 w-80"
            style={{
              left: `${aiTourPosition.x}px`,
              top: `${aiTourPosition.y}px`,
            }}
          >
            {/* 小三角箭头 - 居中指向按钮 */}
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
              <span className="font-semibold text-gray-800">功能提示</span>
              <button
                onClick={handleCloseAITourGuide}
            ...
            <div className="px-4 py-4">
              <p className="text-sm text-gray-600 leading-relaxed">
                点击这里的【智能推荐】按钮，我会根据您的采购信息，自动为您推荐合适的字段值。
              </p>
            </div>
            ...
            <div className="flex justify-end px-4 pb-4">
              <button
                onClick={handleDismissAITourGuide}
