...
const Component = () => {
  ...
  const [onboardingHighlightRect, setOnboardingHighlightRect] = useState<DOMRect | null>(null);
  const onboardingTargetRef = useRef<HTMLElement | null>(null);
  // 新手引导预览模式 - 第3步时显示"荐"标签预览
  const isOnboardingPreviewMode = showOnboarding && currentOnboardingStep === 2;
  ...
  const onboardingSteps = [
    {
      id: 'welcome',
      title: '欢迎使用智能推荐',
      content: '您好！我是小云，您的AI智能助手。我可以帮您快速完成编制工作，让编写文件，填写表单更轻松！',
      target: null, // 无具体高亮目标，居中显示
      position: 'center',
    },
    {
      id: 'ai-button',
      title: '智能推荐入口',
      content: '点击这里的【智能推荐】按钮，我会根据您的采购信息，自动为您推荐合适的字段值。',
      target: 'ai-recommend-button',
      position: 'right',
    },
    {
      id: 'form-fields',
      title: '推荐效果预览',
      content: '推荐完成后，我会为每个推荐字段标记"荐"标签，您可以随时修改，修改后标签会自动消失。',
      target: 'field-autoPublish',
      position: 'right',
    },
    {
      id: 'complete',
      title: '开始使用吧',
      content: '现在您可以开始使用智能推荐功能了！',
      target: null,
      position: 'center',
    },
  ];
  ...
  const calculateOnboardingPosition = () => {
    ...
    if (!step.target || !onboardingHighlightRect) {
      // 居中显示
      return {
        x: Math.max(20, (window.innerWidth - bubbleWidth) / 2),
        y: Math.max(20, (window.innerHeight - bubbleHeight) / 2),
      };
    }

    // 根据步骤决定位置
    const isStep2 = step.id === 'ai-button';
    ...
    if (isStep2) {
      // 第2步（智能推荐按钮）：显示在按钮下方
      x = onboardingHighlightRect.left + (onboardingHighlightRect.width - bubbleWidth) / 2;
      y = onboardingHighlightRect.bottom + padding;
      ...
      // 如果下方空间不足，则显示在上方
      if (y + bubbleHeight > window.innerHeight) {
        y = onboardingHighlightRect.top - bubbleHeight - padding;
      }
    } else {
      // 其他步骤：默认显示在目标右侧
      x = onboardingHighlightRect.right + padding;
      y = onboardingHighlightRect.top;
      
      // 检测右侧是否超出屏幕
      if (x + bubbleWidth > window.innerWidth) {
        x = onboardingHighlightRect.left - bubbleWidth - padding;
      }
    ...
    // 确保不会超出边界
    x = Math.max(padding, Math.min(x, window.innerWidth - bubbleWidth - padding));
    y = Math.max(padding, Math.min(y, window.innerHeight - bubbleHeight - padding));
  ...
  useEffect(() => {
    ...
    if (step.target) {
      ...
      if (element) {
        onboardingTargetRef.current = element;

        // 先滚动到目标元素
        element.scrollIntoView({ behavior: 'smooth', block: 'center' });

        // 等待滚动动画完成后再计算位置（500ms 是 scrollIntoView 动画的大致时间）
        const timer = setTimeout(() => {
          const rect = element.getBoundingClientRect();
          setOnboardingHighlightRect(rect);
        }, 550);
      ...
      setOnboardingHighlightRect(null);
      onboardingTargetRef.current = null;
    }
              ...
              <div className="p-4">
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  >
                    ...
                    {/* AI推荐标签 - 实际推荐或新手引导预览模式都显示 */}
                    {(aiRecommendedFields['autoPublish'] || isOnboardingPreviewMode) && (
                  ...
                  >
                    ...
                    {/* AI推荐标签 - 实际推荐或新手引导预览模式都显示 */}
                    {(aiRecommendedFields['hoursAfterApproval'] || isOnboardingPreviewMode) && (
                      <span className="ml-2 px-1.5 py-0.5 bg-blue-100 text-blue-600 text-xs rounded border border-blue-200">
                        荐
                      </span>
                    )}
                ...
                >
                  <div className="flex items-center mb-2">
                    ...
                    {/* AI推荐标签 - 实际推荐或新手引导预览模式都显示 */}
                    {(aiRecommendedFields['noticeFormat'] || isOnboardingPreviewMode) && (
                      <span className="ml-2 px-1.5 py-0.5 bg-blue-100 text-blue-600 text-xs rounded border border-blue-200">
                        荐
                      </span>
                    )}
                ...
                >
                  <div className="flex items-center">
                    ...
                    {/* AI推荐标签 - 实际推荐或新手引导预览模式都显示 */}
                    {(aiRecommendedFields['noticeContent'] || isOnboardingPreviewMode) && (
                      <span className="ml-2 px-1.5 py-0.5 bg-blue-100 text-blue-600 text-xs rounded border border-blue-200">
                        荐
                      </span>
                    )}
        ...
        <div className="fixed inset-0 z-[60] pointer-events-auto">
            ...
            <defs>
              <mask id="onboarding-mask">
                ...
                {onboardingHighlightRect && (
                  <rect
                    x={onboardingHighlightRect.left - 8}
                    y={onboardingHighlightRect.top - 8}
                    width={onboardingHighlightRect.width + 16}
                    height={onboardingHighlightRect.height + 16}
                    rx="12"
                    fill="black"
                  />
                )}
          ...
          {onboardingHighlightRect && (
            <div
              ...
              style={{
                left: onboardingHighlightRect.left - 8,
                top: onboardingHighlightRect.top - 8,
                width: onboardingHighlightRect.width + 16,
                height: onboardingHighlightRect.height + 16,
                borderRadius: '12px',
                border: '3px solid white',
                boxShadow: '0 0 0 4px rgba(59, 130, 246, 0.8), 0 0 20px rgba(59, 130, 246, 0.4)',
              }}
            >
              {/* 脉冲动画效果 */}
              <div className="absolute inset-0 rounded-xl animate-ping bg-blue-400/20"></div>
            </div>
          )}

          {/* 引导气泡 */}
          <div
            className="absolute z-[70] transition-all duration-500 ease-out"
            style={{
              left: calculateOnboardingPosition().x,
              top: calculateOnboardingPosition().y,
            }}
          >
            <div className="flex items-start gap-4">
              {/* AI助手形象 */}
              <div className="flex-shrink-0">
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-400 via-blue-500 to-cyan-400 p-0.5 shadow-xl animate-bounce">
                  <div className="w-full h-full rounded-full bg-white dark:bg-gray-800 flex items-center justify-center overflow-hidden">
                    <img
                      src="https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=cute%20AI%20robot%20mascot%20avatar%20friendly%20smiling%20blue%20gradient&image_size=square"
                      alt="AI小云"
              ...
              {/* 气泡内容 */}
              <div className="relative">
                <div className="bg-white dark:bg-gray-800 rounded-2xl rounded-tl-none px-6 py-5 shadow-2xl border border-blue-100 dark:border-blue-800 w-[320px]">
                  {/* 步骤指示器 */}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="px-4 py-1.5 text-xs bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded hover:from-blue-600 hover:to-cyan-600 transition-all shadow-sm"
                      >
                        {currentOnboardingStep === onboardingSteps.length - 1 ? '开始使用' : '下一步'}
                      </button>
                    </div>
                  </div>
                </div>
                {/* 气泡小三角 - 根据步骤调整方向 */}
                {onboardingSteps[currentOnboardingStep].id === 'ai-button' ? (
                  // 第2步：小三角在上方，指向按钮
                  <div className="absolute left-1/2 -translate-x-1/2 -top-2 w-4 h-4 bg-white dark:bg-gray-800 border-l border-t border-blue-100 dark:border-blue-800 transform rotate-45"></div>
                ) : (
                  // 其他步骤：小三角在左侧
                  <div className="absolute left-[-8px] top-6 w-4 h-4 bg-white dark:bg-gray-800 border-l border-b border-blue-100 dark:border-blue-800 transform rotate-45"></div>
                )}
              </div>
            </div>
          </div>
