...
- 背景：白色，底部 1px 边框
- 右侧对齐：下载中心、用户信息按钮
- 使用 Ant Design Button 组件，文本按钮样式

**左侧边栏**：
- 宽度：200px（展开）/ 60px（折叠）
- 背景：白色
- 顶部 Logo：统一使用运营后台 logo（`/assets/media/运营后台左上角logo.png`）
- 菜单结构：主菜单 + 子菜单
- 菜单项高度：40px（主菜单）/ 36px（子菜单）
- 选中状态：背景 #e6f7ff，文字 #1890ff
- 折叠按钮：固定在侧边栏底部，宽度 100%，带顶部边框
...
**设计规范文档**：`src/docs/ai-recommendation-pattern.md`

**核心要素**：
- 触发按钮：紫色渐变，带Sparkles图标
- 漫游式引导：字段高亮（黄色背景+边框+阴影）
- AI小人：渐变边框圆形头像，带弹跳动画
- 气泡提示：显示推荐进度和当前操作
