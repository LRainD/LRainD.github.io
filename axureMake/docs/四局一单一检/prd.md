...
| V0.31 | 2026-09-01 | 删除多角色身份切换能力，同一账号的角色、人员资格和任务关系直接叠加展示，并按具体业务关系、组织范围和流程状态逐项授权 |
| V0.32 | 2026-09-01 | 统一阶段完成状态为CS-010“已自查”、CS-020“已互查”和CS-030“已稽查”，并同步流程、规则和页面口径 |
...
2. **检查办理类操作不配置静态权限点。**用户能否执行自查、互查、稽查，由“有效的合规人员资格 + 任务已分派给本人 + 单据当前检查状态允许操作”共同决定。
...
| 局管理员     | 组织数据范围和现有平台规则允许的检查任务记录，以及分派给本人的自查、互查、稽查申诉 | 局管理员角色、组织数据范围及申诉任务关系 | 查看、导出和申诉处理操作需留痕                   |
...
2. 用户作为某类任务的检查人、原检查人或历史办理人时，即使对应人员资格已停用或被移出人员库，仍按业务关系展示该类型页签及其历史任务；局管理员在存在自查、互查或稽查申诉待办、已办关系时展示对应任务。
...
| CS-010 | 已自查       | 自查已完成且结论合规，或全部自查问题已闭环；可进入互查候选池 | 互查待派发、互查待处理                      |
...
| CS-020 | 已互查       | 互查已完成且结论合规，或全部互查问题已闭环；可进入稽查候选池 | 稽查待派发、稽查待处理                      |
...
10. 处于CS-010“已自查”或CS-020“已互查”且长期未被抽中的单据保持当前状态，不自动进入下一阶段。
...
| BR-034 | 检查办理不配置静态权限点，由有效人员资格、任务分派关系和流程状态共同授权                       |
...
统一工作台、统一用户、组织主数据、统一权限、消息通知、附件存储和操作审计等属于基础服务或平台支撑能力。本次仅描述其与一单一检的集成边界，不将基础服务页面作为本次重点建设页面。
...
| F-SELF-006  | 自查管理 | 自查申诉处理       | 局管理员                 | P0    | 按检查项作出最终结论；未闭环问题退回继续整改，全部闭环后更新为CS-010“已自查”        |
...
- 管理类菜单和操作按钮按RBAC权限点展示；检查办理入口按人员资格和本人任务动态展示。
...
| 单位供应链负责人 | PG-001 统一工作台 | PG-020、PG-021、PG-069、PG-004；配置页面按权限点显示                                               |
| 合规专员     | PG-001 统一工作台 | PG-002、PG-014、PG-004                              |
| 合规专家     | PG-001 统一工作台 | PG-002、PG-014、PG-004                              |
| 稽查专家     | PG-001 统一工作台 | PG-002、PG-014、PG-004                       |
| 局管理员     | PG-001 统一工作台 | PG-002、PG-014、PG-020、PG-021、PG-070 至 PG-077、PG-090、PG-004；其中配置页面按权限点显示 |
...
- 用户为局管理员时，在自查、互查、稽查页签展示分派给本人的申诉处理任务。
...
- 互查：展示两类数据：一是处于“已自查”的候选合同，此时尚未创建互查任务；二是已发起互查并处于“互查待派发”的单据数据，此时已经创建互查任务但尚未确定合规专家。候选合同必须自查合规或全部自查问题已通过整改复核或申诉闭环，且不存在待处置、待复核、复核不通过或申诉处理中；仍处于自查问题闭环的合同不得展示。支持单条或批量发起互查，发起后从“已自查”进入“互查待派发”。
- 稽查：展示两类数据：一是处于“已互查”的候选合同，此时尚未创建稽查任务；二是已发起稽查并处于“稽查待派发”的单据数据，此时已经创建稽查任务但尚未确定稽查专家。候选合同必须互查合规或全部互查问题已闭环，且不存在待处置、待复核、复核不通过或申诉处理中；展示互查问题闭环状态和原互查人员，禁止将未闭环问题加入稽查批次。
...
| 互查 | 已自查；表示自查已完成并满足互查准入，尚未创建互查任务 | 互查待派发；已创建互查任务但未确定合规专家 | 互查待处理及后续状态的派发和批次记录 | 互查回退审批通过后进入互查待派发 |
| 稽查 | 已互查；表示互查已完成并满足稽查准入，尚未创建稽查任务 | 稽查待派发；已创建稽查任务但未确定稽查专家 | 稽查待处理及后续状态的派发和批次记录 | 稽查回退审批通过后进入稽查待派发 |
...
8. 最终确认前再次校验权限、组织范围、合同状态、准入条件、批次锁定、专家资格、跨单位回避和幂等请求。直接派发成功时“已自查”进入“互查待处理”；选择人工指定但暂不完成派发时进入“互查待派发”。
9. 批量部分成功时，成功合同进入“互查待处理”，失败合同保留“已自查”或根据已创建任务进入“互查待派发”，未抽中的候选继续保持“已自查”；批次详情必须展示逐条失败原因，不得整体回滚合法成功结果。

**互查待派发处理及调整**：

- “待派发”视图只展示“互查待派发”，包括发起互查时选择暂不指定专家、系统均衡派发失败、回退审批通过后等待重新派发的任务，不混入尚未发起的“已自查”候选合同。
...
1. 进入“待发起”视图，按所属单位、风险等级、互查闭环状态、是否已有稽查任务、互查提交时间和进入已互查状态时间筛选候选合同；展示互查任务编号、互查检查人、问题总数、已闭环数、未闭环数、互查申诉状态和稽查准入结果。
2. 勾选单条或批量候选合同，设置抽取范围及派发方式。
3. 系统校验合同当前为“已互查”、互查合规或全部问题已闭环、不存在待处置/待复核/复核不通过/申诉处理中、未生成有效稽查任务且未被其他批次锁定；不符合条件的合同显示具体原因。
...
9. 最终确认前再次校验当前状态仍为“已互查”、互查问题仍已全部闭环、专家资格有效、双重回避通过、批次未重复创建且幂等请求有效。直接派发成功时“已互查”进入“稽查待处理”；先发起待人工派发时进入“稽查待派发”。
10. 未抽中的合同继续保持“已互查”；批量部分成功时成功合同进入“稽查待处理”，失败和未处理合同在批次详情中分组展示并支持针对失败明细重试，不得因部分失败回滚成功任务。

**稽查待派发处理及调整**：

- “待派发”视图只展示“稽查待派发”，包括发起稽查时选择暂不指定专家、系统均衡派发失败、回退审批通过后等待重新派发的任务，不混入尚未发起的“已互查”候选合同。
