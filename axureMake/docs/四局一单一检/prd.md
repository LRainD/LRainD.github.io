...
| V0.13 | 2026-08-31 | 重构状态模型，以状态机展示检查任务、阶段准入、问题整改、申诉和任务回退状态流转                   |
...
| V0.18 | 2026-08-31 | 重构状态模型章节，增加状态总表和检查任务总体流转图，取消检查阶段、准入和任务状态的重复独立状态机，仅保留复杂子流程状态机   |
| V0.19 | 2026-08-31 | 将检查任务状态收敛为一套统一主状态，每个状态独立编码并在同一张状态机图中流转，不再按任务、准入、问题、申诉和回退拆分子状态机 |
| V0.20 | 2026-08-31 | 将人物功能用例及用例图移至角色章节，删除报告与分析、非功能性要求、核心验收场景、待确认事项和后续产出建议，并重排后续章节编号 |
| V0.21 | 2026-08-31 | 补齐自查不合规问题的整改与复核流程，将自查申诉调整为整改复核不通过后的可选途径，并同步状态、用例、功能和页面规则 |
| V0.22 | 2026-08-31 | 将首次不合规后的整改与申诉统一为按检查项自主处置，并将整改、复核和申诉办理入口并入“我的任务”自查、互查、稽查页签 |
| V0.23 | 2026-09-01 | 暂不考虑系统判定合规后的人工补充不合规问题，删除对应流程、用例、规则、功能、页面字段、结果计算和审计要求 |
| V0.24 | 2026-09-01 | 删除独立的任务派发与时效章节，将必要说明并入派发任务页面；清理与线下管理事项相关的线上字段和关联能力 |
| V0.25 | 2026-09-01 | 清理页面清单和页面规格中的历史调整痕迹，仅保留当前实际建设的页面结构与功能说明 |
| V0.26 | 2026-09-01 | 合并PG-002“我的任务”的列表办理与检查执行规格，统一页面目标、角色入口、操作列、执行视图及详情跳转口径 |
| V0.27 | 2026-09-01 | 将检查执行、通知与待办规则归入对应页面规格，新增业务侧“任务流转记录”，并将数据留痕与审计要求调整为第13章 |
| V0.28 | 2026-09-01 | 删除重复的权限模型概述图，保留授权原则、权限点目录和数据可见性规则并顺次调整章节编号 |
| V0.29 | 2026-09-01 | 统一全篇使用“申诉”术语，明确自查、互查、稽查申诉均由局管理员处理，删除稽查专家处理申诉及申诉相关分派、回避逻辑 |
| V0.30 | 2026-09-01 | 统一页面规格标题格式，标题仅保留页面名称，页面编码调整为与页面目标同级的独立字段 |
...
4. 支持人工检查项和系统识别项协同执行。
5. 实现原始判定及检查结论不可覆盖，整改、申诉和最终有效结果分层记录。
...
| 有效结果    | 用于提交检查任务及计算当前总体结论的检查项结果；人工检查项取人工判定结果，系统识别项取系统原始结果 |
| 申诉      | 合同经办人针对自查、互查或稽查中的具体不合规检查项提出申诉；首次不合规或整改复核不通过后，在符合规则时按检查项发起，由局管理员处理 |
| 问题闭环    | 不合规问题已通过整改复核或申诉，达到允许进入后续检查阶段的状态                     |
...
| 局管理员     | 发起和管理互查、稽查；审批互查、稽查任务回退；处理自查、互查、稽查申诉并按检查项作出最终结论；查看全局检查任务记录；在获得对应权限点后维护检查模板、角色权限、时限及通知规则等配置 | 权限点角色（RBAC）      |
...
| UC-SC-003 | 复核自查整改   | 本人检查问题已提交整改      | 在“我的任务—自查”进入详情，查看材料并逐项作出复核结论 | 按问题闭环情况流转至自查待处置、复核不通过或待互查状态 |
...
| UC-MC-002 | 复核互查整改   | 本人检查问题已提交整改         | 在“我的任务—互查”进入详情，查看材料并作出复核结论 | 按是否全部闭环流转至互查待处置、复核不通过或待稽查状态 |
...
| UC-A-004 | 处理申诉      | 收到自查、互查或稽查申诉           | 在“我的任务”对应页签进入详情，按检查项审查材料并作出最终结论 | 申诉通过则问题闭环；申诉不通过则转为必须整改，全部问题闭环后进入下一阶段或检查完成 |
...
3. **整改、复核和申诉由业务关系控制。**合同经办人处理本人合同的整改和申诉；原检查人处理对应整改复核，局管理员处理自查、互查、稽查申诉。
...
以下操作不设置独立静态权限点：执行本人检查任务、保存检查草稿、提交本人检查、申请本人任务回退、合同经办人整改或发起申诉、原检查人复核整改。系统通过人员资格、任务关系、业务关系和流程状态实时校验。
...
| 局管理员     | 组织数据范围和现有平台规则允许的检查任务记录，以及分派给本人的自查、互查、稽查申诉 | 局级管理身份、组织数据范围及申诉任务关系 | 查看、导出和申诉处理操作需留痕                   |
| 人员库管理员   | 资格所属组织在授权范围内的人员资格记录      | 人员库权限点 + 资格所属组织数据范围     | 账号所属组织不决定其维护权限；不能因人员库管理权限办理检查任务 |

“我的任务”页面按检查类型设置“自查、互查、稽查”三个页签。页签展示遵循以下规则：

1. 当前身份为合同经办人时，按本人负责合同实际存在的检查类型展示“自查、互查、稽查”页签；当前存在启用的合规专员、合规专家或稽查专家资格时，分别展示与资格对应的检查类型页签。
2. 用户作为某类任务的检查人、原检查人或历史办理人时，即使对应人员资格已停用、被移出人员库或当前业务身份已失效，仍按业务关系展示该类型页签及其历史任务；局管理员在存在自查、互查或稽查申诉待办、已办关系时展示对应任务。
3. 人员资格失效后不得接收新任务；若存量任务仍分派给本人且当前状态允许办理，用户按任务生成时保存的资格快照继续完成任务。已被收回、回退或重新派发给他人的任务只读展示，不允许原检查人继续编辑或提交。
4. 用户在当前身份下既无合同经办、有效资格、历史办理关系，局管理员也无申诉待办或已办关系时，不展示对应页签；三个页签均不满足展示条件时，页面显示“暂无可查看的任务”，不默认暴露任何任务数据。
...
| CS-003 | 自查中       | 合规专员正在执行人工检查或系统检测         | 自查回退审批中、自查已收回、自查待处置、待互查 |
...
| CS-008 | 自查待复核     | 合同经办人已提交整改，等待原自查合规专员复核     | 自查待处置、自查复核不通过、待互查               |
| CS-009 | 自查复核不通过   | 原自查合规专员复核不通过，等待合同经办人继续处置    | 自查待处置、自查待复核、自查申诉中             |
| CS-031 | 自查申诉中     | 局管理员正在按检查项处理自查申诉          | 自查待处置、自查待复核、自查复核不通过、待互查        |
| CS-010 | 待互查       | 自查合规或全部自查问题已闭环，进入互查候选池     | 互查待派发、互查待处理                      |
| CS-011 | 互查待派发     | 已发起互查但尚未确定合规专家            | 互查待处理                             |
| CS-012 | 互查待处理     | 已确定合规专家，尚未开始办理            | 互查中、互查回退审批中                      |
| CS-013 | 互查中       | 合规专家正在执行互查                | 互查回退审批中、互查待处置、待稽查        |
| CS-015 | 互查回退审批中   | 互查任务回退申请等待局管理员审批          | 互查待派发、互查待处理、互查中               |
| CS-016 | 互查待处置     | 互查存在尚未闭环的不合规检查项，合同经办人可按项整改或在具备资格时申诉 | 互查待复核、互查申诉中                 |
| CS-017 | 互查待复核     | 合同经办人已提交整改，等待原检查人复核       | 互查待处置、互查复核不通过、待稽查               |
| CS-018 | 互查复核不通过   | 原检查人复核不通过，等待合同经办人继续处置      | 互查待处置、互查待复核、互查申诉中               |
| CS-019 | 互查申诉中     | 局管理员正在按检查项处理互查申诉          | 互查待处置、互查待复核、互查复核不通过、待稽查        |
| CS-020 | 待稽查       | 互查合规或全部互查问题已闭环，进入稽查候选池    | 稽查待派发、稽查待处理                      |
| CS-021 | 稽查待派发     | 已发起稽查但尚未确定稽查专家            | 稽查待处理                             |
| CS-022 | 稽查待处理     | 已确定稽查专家，尚未开始办理            | 稽查中、稽查回退审批中                      |
| CS-023 | 稽查中       | 稽查专家正在执行稽查                | 稽查回退审批中、稽查待处置、检查已完成        |
| CS-025 | 稽查回退审批中   | 稽查任务回退申请等待局管理员审批          | 稽查待派发、稽查待处理、稽查中               |
| CS-026 | 稽查待处置     | 稽查存在尚未闭环的不合规检查项，合同经办人可按项整改或在具备资格时申诉 | 稽查待复核、稽查申诉中                 |
| CS-027 | 稽查待复核     | 合同经办人已提交整改，等待原检查人复核       | 稽查待处置、稽查复核不通过、检查已完成               |
| CS-028 | 稽查复核不通过   | 原检查人复核不通过，等待合同经办人继续处置      | 稽查待处置、稽查待复核、稽查申诉中               |
| CS-029 | 稽查申诉中     | 局管理员正在按检查项处理稽查申诉          | 稽查待处置、稽查待复核、稽查复核不通过、检查已完成        |
...
2. 回退审批通过后进入对应“待派发”状态；驳回或24小时超时自动驳回后，返回申请回退前的待处理或检查中状态。
3. 仅自查超过办结时限后进入CS-006“自查已收回”；互查、稽查超时只提醒、催办和留痕，当前检查状态不变化。
4. 自查按问题逐项整改、复核和申诉；只有全部问题闭环，单据才进入CS-010“待互查”，仍有任一未闭环问题时继续停留在自查整改闭环状态。
5. 互查、稽查同样按问题逐项整改、复核和申诉；只有全部问题闭环，单据才分别进入CS-020“待稽查”或CS-030“检查已完成”。
...
8. 每个不合规检查项首次进入待处置时，经办人可选择整改或申诉；申诉不通过后该检查项标记为“必须整改”，不得基于同一原始检查结论重复申诉。整改复核不通过后，仅此前未申诉过且仍在申诉时限内的检查项可发起一次申诉。
9. 原始判定结果和原始检查结论不因整改或申诉发生变化，状态流转只改变当前检查状态和相应检查项的有效结果、闭环结果。
...
| BR-006 | 自查、互查、稽查申诉均由局管理员按检查项作出最终结论；申诉不通过的检查项转为必须整改，逾期未申诉的检查项仍可整改       |
| BR-007 | 互查人员不得查看自查结果                                                |
...
| BR-016 | 自查、互查、稽查问题经整改复核或申诉通过后视为闭环                                  |
| BR-017 | 申诉不通过或逾期未申诉时，自查、互查、稽查问题不自动关闭；申诉不通过的检查项必须整改             |
| BR-018 | 所有必检项完成后方可提交检查任务                                 |
| BR-019 | 人工检查项必须选择合规或不合规；不合规时问题原因和制度依据必填                             |
| BR-020 | 系统识别项必须取得系统原始结果，并以该结果参与任务提交和总体结论计算            |
...
| BR-040 | 整改、复核和申诉根据合同、当前检查人、原检查人及申诉任务等业务关系授权；申诉处理仅授权局管理员                       |
...
| BR-055 | “我的任务”统一承载经办人的不合规处置、原检查人的整改复核以及局管理员的申诉待办，并按自查、互查、稽查页签和当前业务身份隔离 |
...
角色工作台、统一用户、组织主数据、统一权限、消息通知、附件存储和操作审计等属于基础服务或平台支撑能力。本次仅描述其与一单一检的集成边界，不将基础服务页面作为本次重点建设页面。
...
| F-BASE-006  | 基础服务 | 多角色身份切换      | 多角色用户                | 基础服务 | 对接基础工作台和身份服务，本期不展开页面说明          |
...
| F-BASE-014  | 配置管理 | 派发候选人员过滤     | 系统                   | P0    | 按人员类型、资格所属组织和启用状态生成候选池，再按账号所属组织及业务关系执行回避           |
...
| F-SELF-004  | 自查管理 | 我的任务—自查页签    | 合同经办人、合规专员、局管理员、历史办理人 | P0    | 统一展示自查检查、待处置、待复核、申诉处理及历史任务                         |
| F-SELF-005  | 自查管理 | 自查申诉         | 合同经办人                | P0    | 对首次不合规或复核不通过、此前未申诉且仍在时限内的自查检查项发起申诉              |
| F-SELF-006  | 自查管理 | 自查申诉处理       | 局管理员                 | P0    | 按检查项作出最终结论；未闭环问题退回继续整改，全部闭环后更新为CS-010“待互查”        |
...
| F-REC-002   | 整改闭环 | 单合同整改        | 合同经办人                | P0    | 从“我的任务”进入任务详情，按检查项提交整改说明和材料                         |
| F-REC-003   | 整改闭环 | 批量整改         | 合同经办人                | P1    | 对多份合同批量提交整改材料，待业务确认                                |
| F-REC-004   | 整改闭环 | 整改复核         | 原检查人                 | P0    | 在“我的任务”对应页签进入详情，复核整改并作出通过或不通过结论                    |
| F-APP-001   | 申诉   | 统一申诉对象及状态    | 系统                   | P0    | 按“检查任务 + 检查项 + 原始检查结论”记录申诉类型、发起人、材料、处理人、状态和结论      |
| F-APP-003   | 申诉   | 自查申诉         | 合同经办人、局管理员           | P0    | 首次不合规或整改复核不通过后，在“我的任务—自查”按符合资格的检查项发起、处理申诉       |
| F-APP-004   | 申诉   | 互查申诉         | 合同经办人                | P0    | 首次不合规或整改复核不通过后，在“我的任务—互查”按符合资格的检查项发起申诉           |
| F-APP-005   | 申诉   | 互查申诉处理       | 局管理员                 | P0    | 按检查项作出互查申诉结论并更新问题闭环状态                            |
| F-APP-006   | 申诉   | 稽查申诉         | 合同经办人                | P0    | 首次不合规或整改复核不通过后，在“我的任务—稽查”按符合资格的检查项发起申诉           |
| F-APP-007   | 申诉   | 稽查申诉处理       | 局管理员                 | P0    | 按检查项作出稽查申诉结论并更新问题闭环状态                            |
...
| F-QUERY-001 | 查询权限 | 我的任务         | 合同经办人、各检查人、局管理员、历史办理人 | P0    | 按自查、互查、稽查页签统一展示检查执行、不合规处置、整改复核、申诉处理及历史任务，并按身份提供对应操作 |
| F-QUERY-002 | 查询权限 | 经办检查任务记录     | 合同经办人                | P0    | 查看本人合同的检查任务、整改和申诉记录                                |
...
| 页面范围 | 主要关联功能编号 |
| --- | --- |
| PG-001、PG-002、PG-004 工作台 | F-QUERY-001、F-QUERY-003、F-NOTI-001、F-NOTI-002、F-BASE-006、F-EXEC-001 至 F-EXEC-008、F-SYS-001 至 F-SYS-003、F-TASK-001、F-REC-001 至 F-REC-004、F-APP-001、F-APP-003 至 F-APP-007 |
| PG-010、PG-011、PG-014 检查任务 | F-QUERY-002、F-QUERY-003、F-OBJ-001 至 F-OBJ-003、F-REC-002、F-REC-004、F-APP-003 至 F-APP-007 |
| PG-020 派发任务 | F-QUERY-003、F-SELF-001 至 F-SELF-003、F-MUT-001 至 F-MUT-006、F-AUD-001、F-AUD-002、F-AUD-004、F-AUD-005、F-TASK-002、F-TASK-003、F-TASK-005、F-TASK-008 |
| PG-069 至 PG-079 配置管理 | F-BASE-001 至 F-BASE-006、F-BASE-008 至 F-BASE-021 |
...
- “我的任务”统一承载检查执行、不合规处置、整改复核和申诉处理；自查、互查、稽查通过顶部页签、当前业务身份、任务关系和状态控制展示及操作。
...
| ------ | ----- | ----------- | ----- | ---------------- | --- | -------------------------------- |
| PG-001 | 工作台   | 角色工作台       | 基础服务入口 | 全部登录用户           | 基础服务 | 使用基础服务提供的统一工作台，本期不展开页面说明           |
...
| PG-010 | 检查任务  | 经办检查任务记录    | 页面    | 合同经办人            | P0  | 查询本人负责合同的全部检查任务状态、结论和历史记录；待办操作统一进入PG-002 |
...
| 角色       | 默认首页         | 主要业务页面                                                                               |
| -------- | ------------ | ------------------------------------------------------------------------------------ |
| 合同经办人    | PG-001 角色工作台 | PG-002、PG-010、PG-011、PG-014、PG-004                                     |
| 单位供应链负责人 | PG-001 角色工作台 | PG-020、PG-021、PG-069、PG-004；配置页面按权限点显示                                               |
| 合规专员     | PG-001 角色工作台 | PG-002、PG-014、PG-004                              |
| 合规专家     | PG-001 角色工作台 | PG-002、PG-014、PG-004                              |
| 稽查专家     | PG-001 角色工作台 | PG-002、PG-014、PG-004                       |
| 局管理员     | PG-001 角色工作台 | PG-002、PG-014、PG-020、PG-021、PG-070 至 PG-077、PG-090、PG-004；其中配置页面按权限点显示 |
...
- 多角色用户先选择当前业务身份，包括合同经办人、合规专员、合规专家、稽查专家、局管理员等；查询、字段可见性和操作权限按当前身份、任务关系及组织范围隔离。
- 每个业务身份下均按自查、互查、稽查设置页签；没有对应任务关系的页签不展示，三类页签均无任务时展示空状态。
- 合同经办人身份：展示本人负责合同中处于待处置、待复核、复核不通过、申诉中及已闭环状态的任务；首次不合规后进入对应页签待办。
- 检查人身份：展示分派给本人的检查执行任务，以及由本人原检查结论产生的整改复核任务；互查人员仍不得查看自查结果，稽查人员按权限查看关联历史。
- 局管理员身份：在自查、互查、稽查页签展示分派给本人的申诉处理任务。
...
**检查人及局管理员操作列**：原检查人存在待复核事项时显示“复核”；局管理员存在申诉待办时显示“处理申诉”；检查任务尚未提交时按原规则显示“开始检查”或“继续检查”。
...
| 有效结果    | 人工检查项取人工判定结果，系统识别项取系统原始结果                     |
| 检查备注    | 按模板配置校验非必填、始终必填或不合规时必填                           |
| 检查附件    | 按模板配置校验非必填、始终必填或不合规时必填                           |
| 问题原因    | 人工不合规时必填                                         |
| 制度依据    | 人工不合规时必填                                         |
| 申诉状态    | 未提出、处理中、全部通过、部分通过、全部不通过；仅已进入申诉条件的不合规检查项展示 |
...
| 检查方式 | 原始结果 | 最终有效结果 |
| ---- | ---- | ------ |
| 人工检查 | 人工判定为合规或不合规 | 与人工判定结果一致 |
| 系统识别 | 系统返回合规或不合规 | 与系统原始结果一致 |
| 共享引用 | 来源检查项的有效结果 | 与合法引用的来源结果一致 |
...
6. 申诉通过时对应检查项闭环；申诉不通过时标记为必须整改，详情页隐藏该项“发起申诉”操作，仅保留“提交整改”。
7. 原检查人在同一详情处置区逐项复核整改；局管理员处理自查、互查、稽查申诉，均不得修改原始判定结果和原始检查结论。

**任务流转记录**：

- PG-014详情设置独立的“任务流转记录”区，按时间倒序展示当前任务的关键业务事件，不记录普通查看、页面访问和草稿自动保存等低价值操作。
- 记录范围包括任务生成、发起、派发、调整检查人、开始处理、检查提交、回退申请及审批、超时、自动收回、重新派发、整改提交、整改复核、申诉提交及处理、问题闭环和进入下一检查阶段。
...
- 互查：展示两类数据：一是处于“待互查”的候选合同，此时尚未创建互查任务；二是已发起互查并处于“互查待派发”的单据数据，此时已经创建互查任务但尚未确定合规专家。候选合同必须自查合规或全部自查问题已通过整改复核或申诉闭环，且不存在待处置、待复核、复核不通过或申诉处理中；仍处于自查问题闭环的合同不得展示。支持单条或批量发起互查，发起后从“待互查”进入“互查待派发”。
- 稽查：展示两类数据：一是处于“待稽查”的候选合同，此时尚未创建稽查任务；二是已发起稽查并处于“稽查待派发”的单据数据，此时已经创建稽查任务但尚未确定稽查专家。候选合同必须互查合规或全部互查问题已闭环，且不存在待处置、待复核、复核不通过或申诉处理中；展示互查问题闭环状态和原互查人员，禁止将未闭环问题加入稽查批次。
...
| 互查 | 待互查；仅表示满足互查准入，尚未创建互查任务 | 互查待派发；已创建互查任务但未确定合规专家 | 互查待处理及后续状态的派发和批次记录 | 互查回退审批通过后进入互查待派发 |
| 稽查 | 待稽查；仅表示满足稽查准入，尚未创建稽查任务 | 稽查待派发；已创建稽查任务但未确定稽查专家 | 稽查待处理及后续状态的派发和批次记录 | 稽查回退审批通过后进入稽查待派发 |

“待发起”“已派发”“派发异常”“批次处理中”等仅为页面视图、批次状态或派发结果，不得新增为检查任务主状态；任务当前状态必须直接使用第9章统一状态。

**互查操作**：

1. 进入“待发起”视图，按所属单位、合同时间、合同属性、风险等级、自查最终准入结果、是否已有互查任务和进入待互查时间筛选候选合同；支持查看自查任务编号、自查提交时间、问题闭环进度、整改复核状态、申诉处理状态及未处理事项提示。
2. 单条勾选或批量勾选合同，系统重新校验合同当前仍为“待互查”、自查合规或全部自查问题已闭环、无有效互查任务且未被其他批次锁定；不符合条件的合同列出原因并不能进入下一步。
...
6. 候选合规专家必须人员类型为合规专家、资格状态为启用、资格所属组织符合派发范围，且账号所属组织不得与合同所属单位相同。候选列表展示姓名、账号、账号所属组织、资格所属组织、当前待办量、进行中任务量、回避状态和不可选原因。
7. 展示合同与专家分配预览，包括合同所属单位、目标专家、专家组织、待办量、跨单位回避结果、预计办结时间和无法派发原因；人工调整后必须重新执行回避校验。
8. 最终确认前再次校验权限、组织范围、合同状态、准入条件、批次锁定、专家资格、跨单位回避和幂等请求。直接派发成功时“待互查”进入“互查待处理”；选择人工指定但暂不完成派发时进入“互查待派发”。
9. 批量部分成功时，成功合同进入“互查待处理”，失败合同保留“待互查”或根据已创建任务进入“互查待派发”，未抽中的候选继续保持“待互查”；批次详情必须展示逐条失败原因，不得整体回滚合法成功结果。

**互查待派发处理及调整**：

- “待派发”视图只展示“互查待派发”，包括发起互查时选择暂不指定专家、系统均衡派发失败、回退审批通过后等待重新派发的任务，不混入尚未发起的“待互查”候选合同。
- 具备“发起互查”但不具备“互查派发管理”权限时，可完成候选抽取和批次创建，但不得人工指定或调整专家；具备“互查派发管理”但不具备“发起互查”权限时，只能处理已经处于“互查待派发”的任务和允许调整的已派发任务。
- 从“待派发”视图指定专家时，展示原批次、抽取规则、发起人、发起时间、派发失败原因和历史检查人，重新加载有效合规专家并执行跨单位回避；成功后从“互查待派发”进入“互查待处理”。
- 调整检查人仅允许“互查待处理”且尚未开始办理的任务；“互查中”必须通过回退审批后重新派发，“互查回退审批中”以及互查结果已提交后的状态禁止普通调整。调整前后均保存人员资格快照、回避结果、调整原因和通知记录。

**稽查操作**：

1. 进入“待发起”视图，按所属单位、风险等级、互查闭环状态、是否已有稽查任务、互查提交时间和进入待稽查时间筛选候选合同；展示互查任务编号、互查检查人、问题总数、已闭环数、未闭环数、互查申诉状态和稽查准入结果。
2. 勾选单条或批量候选合同，设置抽取范围及派发方式。
3. 系统校验合同当前为“待稽查”、互查合规或全部问题已闭环、不存在待处置/待复核/复核不通过/申诉处理中、未生成有效稽查任务且未被其他批次锁定；不符合条件的合同显示具体原因。
...
6. 候选稽查专家必须人员类型为稽查专家、资格状态为启用、资格所属组织符合派发范围，账号所属组织不得与合同所属单位相同，且不得是该合同原互查人员。
7. 执行双重回避：第一重比较专家账号所属组织与合同所属单位，第二重比较专家账号与该合同原互查人员账号；同一用户拥有多条资格时，只要账号命中原互查人员，所有对应资格均不可用于该合同。人工调整后必须再次执行两项回避校验。
8. 预览合同、互查闭环状态、原互查人员、稽查专家、专家账号所属组织、资格所属组织、当前待办量、两项回避结果、综合校验结果和失败原因；无法通过回避的合同不得确认派发。
9. 最终确认前再次校验当前状态仍为“待稽查”、互查问题仍已全部闭环、专家资格有效、双重回避通过、批次未重复创建且幂等请求有效。直接派发成功时“待稽查”进入“稽查待处理”；先发起待人工派发时进入“稽查待派发”。
10. 未抽中的合同继续保持“待稽查”；批量部分成功时成功合同进入“稽查待处理”，失败和未处理合同在批次详情中分组展示并支持针对失败明细重试，不得因部分失败回滚成功任务。

**稽查待派发处理及调整**：

- “待派发”视图只展示“稽查待派发”，包括发起稽查时选择暂不指定专家、系统均衡派发失败、回退审批通过后等待重新派发的任务，不混入尚未发起的“待稽查”候选合同。
- 具备“发起稽查”但不具备“稽查派发管理”权限时，可选择候选范围并创建批次，但不得人工指定或调整稽查专家；具备“稽查派发管理”但不具备“发起稽查”权限时，只能处理已经处于“稽查待派发”的任务和允许调整的已派发任务。
- 从“待派发”视图指定专家时，必须展示原批次、原互查人员和历史失败原因，并重新执行所属单位和原互查人员双重回避；成功后从“稽查待派发”进入“稽查待处理”。
...
1. 所有候选检查人均来源于合规人员库，系统按检查类型、人员类型、资格所属组织和启用状态筛选，再执行单位及人员回避，不得直接从全量用户或RBAC角色中选择。
